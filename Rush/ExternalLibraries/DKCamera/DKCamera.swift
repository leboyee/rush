...
//            self.updateFlashMode()
    ...
    fileprivate var defaultPhotoSettings: AVCapturePhotoSettings {
        get {
            if newdefaultPhotoSettings == nil {
                ...
                newdefaultPhotoSettings = photoSettings
            }
    ...
    open func setupUI() {
        ...
        if #available(iOS 11, *) {
            ...
            let safeAreaBottomInset = view.safeAreaInsets.bottom
            bottomViewContainerHeight += safeAreaBottomInset
        }
        ...
        */
        ...
        let cancelButton: UIButton = {
            ...
            let cancelButton = UIButton(frame: CGRect(x: 16, y: navHeight - 21 - 24, width: 24, height: 24))
        ...
        */
        ...
        contentView.addGestureRecognizer(handleZoomGesture)
        contentView.addGestureRecognizer(handleFocusGesture)
    }
    
    var handleZoomGesture: UIPinchGestureRecognizer {
        return UIPinchGestureRecognizer(target: self, action: #selector(DKCamera.handleZoom(_:)))
    }
    
    var handleFocusGesture: UIPinchGestureRecognizer {
        return UIPinchGestureRecognizer(target: self, action: #selector(DKCamera.handleFocus(_:)))
    }
    ...
    @objc open func takePicture() {
        ...
        self.sessionQueue.async {
            self.captureImage { (cropTakenImage, _, error) in
                if let error = error {
...
//                    didFinishCapturingImage(cropTakenImage!, metadata)
                    self.showPreviewCaptureImage(cropTakenImage!)
                }
    ...
    private func readyToCaptureImage() -> Bool {
        if #available(iOS 10.0, *) {
            if  self.captureOutput != nil, self.currentCapturer == nil {
                return true
            } else {
                return false
            }
                    ...
                    if error == nil {
                        ...
                        if let imageData = imageData {
                            ...
                            completeBlock(nil, nil, NSError(domain: "DKCamera", code: -1,
                                                            userInfo: [NSLocalizedDescriptionKey: "DKCamera encountered an Unknown error"]))
                        }
                    } else {
    ...
    open func updateFlashMode() {
        if let currentDevice = self.currentDevice, let captureOutput = self.captureOutput, currentDevice.isFlashAvailable {
            if #available(iOS 10.0, *) {
                if let supported = captureOutput as? AVCapturePhotoOutput {
                    let isFlashModeSupported = supported.__supportedFlashModes.contains(NSNumber(value: self.flashMode.rawValue))
...
extension DKCamera {
    ...
    func showPreviewCaptureImage(_ image: UIImage?) {
        
        if let finalImage = image {
            previewImageView = UIImageView(frame: previewLayer.frame)
            previewImageView.image = finalImage
            self.view.addSubview(previewImageView)
            
            captureButton.isHidden = true
            previewLayer.isHidden = true
            contentView.removeGestureRecognizer(handleFocusGesture)
            contentView.removeGestureRecognizer(handleZoomGesture)
            cameraSwitchButton.setImage(#imageLiteral(resourceName: "camera_finish"), for: .normal)
            cancelButton.setImage(#imageLiteral(resourceName: "back-arrow"), for: .normal)
            cancelButton.tag = 1 // For back navigation
            cameraSwitchButton.tag = 1 // For camera finish capture pic
            ...
            cameraSwitchButton.setImage(cameraResource.cameraSwitchImage(), for: .normal)
            cancelButton.setImage(cameraResource.cameraCancelImage(), for: .normal)
            cancelButton.tag = 0 // For close camera
            cameraSwitchButton.tag = 0 // For camera switching
